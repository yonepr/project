	...
	def reg(message):
		...
		def contact(message):
			if message.contact is not None:
				...
				bot.send_message(message.chat.id, text="Вы можете открыть шлагбаум, нажав на кнопку",reply_markup=a)
				@bot.message_handler(content_types='text')
				def open(message):
...
def adm(message):
	a = telebot.types.ReplyKeyboardRemove()
	bot.send_message(message.chat.id, text="Здравствуйте, {0.first_name}!\nВы перешли в панель администратора.\nДля того что бы удостовериться, что это вы, пожалуйста введите пароль.".format(message.from_user),reply_markup=a)
	@bot.message_handler(content_types='text')
	def nxtadm(message):
